...
export const getStudentGradeByCourse = async (req, res) => {
  try {
    const { courseId } = req.params;
    ...
    const result = components.map((comp) => {
      const grade = grades.find(
        (g) => g.component && g.component._id.toString() === comp._id.toString()
      );
      return {
        component: comp.name,
        weight: comp.weight,
...
export const getGradesForAllCourses = async (req, res) => {
    try {
        const studentId = req.user.userId;
        const courses = await Grade.distinct("course", { student: studentId });
        ...
        const results = await Promise.all(
            courses.map(async (courseId) => {
                ...
                const totalScore = breakdown.reduce((sum, g) => sum + g.score, 0);
